  ...
  if (await Bun.file(htmlPath).exists()) {
    ...
    if (cssFile) {
      html = html.includes('href="tailwindcss"')
        ...
        : html.replace("</head>", `  <link rel="stylesheet" href="/${cssFile}">\n</head>`);
    }
    html = html.includes('src="./main.tsx"')
      ? html.replace('src="./main.tsx"', `src="/${jsFile}"`)
      : html.includes('src="/main.tsx"')
        ? html.replace('src="/main.tsx"', `src="/${jsFile}"`)
        : html.replace("</body>", `  <script type="module" src="/${jsFile}"></script>\n</body>`);
  } else {
    html = [
      "<!DOCTYPE html>",
      '<html lang="en">',
...
export async function buildApplication<TConfig = unknown>(
  ...
  if (existsSync("assets")) await cp("assets", `${dist}/client/assets`, { recursive: true });
  ...
  if (config.mode !== "frontend" && existsSync("app/api")) {
    ...
    for await (const file of glob.scan({ cwd: "app/api" })) apiEntries.push(`app/api/${file}`);
    if (apiEntries.length > 0) {
      await mkdir(`${dist}/api`, { recursive: true });
      const dependencies = Object.keys((await import(`${cwd}/package.json`)).dependencies ?? {});
      await Promise.all(
